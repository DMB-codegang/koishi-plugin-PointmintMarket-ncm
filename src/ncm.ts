...
import { user_account, user_binding, cloudsearch, song_url_v1, TopSongType } from 'netease-cloud-music-api-alger'
...
export class Ncm {
    ...
    async checkCookie(cookie: string): Promise<{ code: number, msg: string }> {
        ...
        if (!cookie || userAccount.account === null || userAccount.profile === null || this.cfg.loginType == 'anonimous') {
            ...
            this.logger.info('Cookie有效，用户昵称：' + userAccount.profile.nickname || '未知')
